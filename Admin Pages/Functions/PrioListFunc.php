...
$queryOrders = "SELECT * FROM (SELECT orders.OrderID, orders.AccountID, orders.OrderPrice, orders.Status, accounts.FirstName, accounts.LastName, accounts.Account_Type, accounts.CourseOrPosition
                                FROM orders INNER JOIN accounts ON orders.AccountID = accounts.AccountID)tb WHERE Account_Type = 'Teacher' OR Account_Type = 'Admin' OR Account_Type = 'Staff' ORDER BY Account_Type";
...
?>
    <div class="col-12 d-flex">
        ...
        <div class="d-flex justify-content-evenly" style="width: 40%;">
            <form method="POST">
                <input type="number" name="OID" class="d-none" value="<?php echo $row['OrderID'] ?>">
                <input type="number" name="AID" class="d-none" value="<?php echo $row['AccountID'] ?>">
                ...
                <button type="submit" name="SCancel" class="btn m-1 rounded-5" style="background-color: #d1a561;" id="Sc<?php echo $row['OrderID'] . $row['AccountID'] ?>">
